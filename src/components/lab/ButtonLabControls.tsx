...
export function ButtonLabControls({ state, onChange }: ButtonLabControlsProps) {
    const selectedIcon = ICON_OPTIONS.find((o) => o.id === state.iconId)?.icon ?? null;

    return (
        <div className="space-y-6 rounded-xl border border-black/10 bg-card p-6 dark:border-white/10">
...
export function ButtonVariantMatrix() {
    return (
        <div className="space-y-4 rounded-xl border border-black/10 bg-card p-6 dark:border-white/10">
                ...
                <table className="w-full min-w-[32rem] text-left text-sm">
                    <thead>
                        <tr className="text-muted-foreground border-b border-black/10 dark:border-white/10">
                            ...
                            {APPEARANCES.map((a) => (
                                <th key={a} className="py-2 px-2 font-medium capitalize">
                                    {a}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {VARIANTS.map((variant) => (
                            <tr key={variant} className="border-b border-black/5 dark:border-white/5">
                                <td className="py-4 pr-4 font-medium capitalize">{variant}</td>
                                {APPEARANCES.map((appearance) => (
                                    <td key={appearance} className="py-4 px-2">
                                        <div className="flex w-2/3 max-w-[8.5rem] flex-col gap-2">
                                            <Button variant={variant} appearance={appearance} className="w-full">
                                                Default
                                            </Button>
                                            <Button variant={variant} appearance={appearance} disabled className="w-full">
                                                Disabled
                                            </Button>
                                            <Button variant={variant} appearance={appearance} loading className="w-full">
                                                Loading
                                            </Button>
                                        </div>
                                    </td>
