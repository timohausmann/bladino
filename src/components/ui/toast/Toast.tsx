...
export function ToastRoot({ children, className, ...props }: ToastRootProps) {
  return (
    <ToastPrimitive.Root
      className={twMerge(
        styles.ToastRoot,
        'text-foreground rounded-lg border border-neutral-200 bg-white px-4 py-3 text-sm shadow-lg',
  ...
  return (
    <ToastPrimitive.Viewport
      className={twMerge(
        ...
        'fixed right-4 bottom-4 z-100 flex max-h-screen w-full max-w-sm flex-col items-end gap-2 outline-none',
        className,
      )}
    />
  );
