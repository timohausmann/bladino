...
import type { ButtonAppearance, ButtonVariant } from '@/components/ui/button/buttonVariants';
...
export function getGlowColor(variant: ButtonVariant, appearance: ButtonAppearance): string {
...
export function ButtonGlow({
    ...
    const [renderPos, setRenderPos] = useState({ x: targetX, y: targetY, radius });
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            opacityRef.current = lerp(opacityRef.current, opacityTarget, OPACITY_LERP);
...
export function getGlowRadius(buttonWidth: number, buttonHeight: number): number {
